...
function Files = GetFiles(payload)
% ---------------------------------------------------------
% NOTE
...
% Init structures(a variable with the following fields: name, folder, data,bytes,isdir,datenum)
...
for i = 1:length(Extensions)
    % Scan for raw files
    RawFiles = [RawFiles; dir(fullfile(payload.path, 'rawdata', 'sub-*', '**', ['sub-*_eeg', Extensions{i}]))]; %#ok<AGROW> 
    % Loop through the derivative modalities and scan for files in each sub-directory
    Modalities = dir([payload.path, '/derivatives']);
    for j = 1:length(Modalities)
        ...
        DerFiles = [DerFiles; dir(fullfile(payload.path, 'derivatives', Modalities(j).name, 'sub-*', '**', ['sub-*', Extensions{i}]))]; %#ok<AGROW> 
    end
    % Finally get all the first-level files
    FstLvlFiles = [FstLvlFiles; dir(fullfile(payload.path, 'derivatives', '*-output-fstlvl', 'sub-*', '**', ['sub-*', Extensions{i}]))]; %#ok<AGROW> 
end
...
startTime = now(); %#ok<TNOW1>
for i = 1:size(payload.subjects_all, 1)
    % ---------------------------------------------------------
    % Set waitbar
    % ---------------------------------------------------------
    % Extract subject id and name
    thisSubjectFileIds = cell(0);
    SubId = payload.subjects_all.Id{i};
    Name = payload.subjects_all.Name{i};
    % ---------------------------------------------------------
    ...
    payload.subjects_all.FileIds{i} = thisSubjectFileIds;
    % ---------------------------------------------------------
    % Command Window
