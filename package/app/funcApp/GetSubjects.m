function Subjects = GetSubjects(payload)
% ---------------------------------------------------------
% Read all the directory names starting with "sub-" in the raw and any
% other derivative's directory
Dirs = dir([payload.path, '/rawdata/sub-*']);
DerivativeDirs = dir([payload.path, '/derivatives/EEG-*']);
for i = 1:length(DerivativeDirs)
    tmp = dir([payload.path, '/derivatives/', DerivativeDirs(i).name, '/sub-*']);
