...
Settings.EEG = LoadDataset(Settings.InputFilePath, 'all');
% ---------------------------------------------------------
...
for i = 1:length(Settings.EEG)
    ...
    if exist([Settings.ProtocolPath, '/derivatives/EEG-select'], 'dir') == 0
        Settings.EEG(i).filepath = [Settings.ProtocolPath, '/derivatives/EEG-preproc/sub-', KeysValues.sub, '/ses-', KeysValues.ses];
    else
        Settings.EEG(i).filepath = [Settings.ProtocolPath, '/derivatives/EEG-select/sub-', KeysValues.sub, '/ses-', KeysValues.ses];
    end
end
% ---------------------------------------------------------
% Store source file
for i = 1:length(Settings.EEG)
    Settings.EEG(i).etc.JSON.Sources = fullpath2bidsuri(Settings.ProtocolPath, Settings.InputFilePath);
end
% ---------------------------------------------------------
% Store history
for i = 1:length(Settings.EEG)
